...
`scan_rhythm.py` считает самый длинный ровный участок внутри куска прозы, а не сквозь весь текст. Раньше `prose_text` склеивал абзацы, выбрасывая заголовки, списки и таблицы, и соседями оказывались фразы из разных секций: участок «8 предложений подряд» на поверку был концом одной секции и началом следующей. Разбор на куски вынесен в `rutext.prose_segments`.

`baseline.json` пересчитан на корпусе из 122 постов: `flat_run` не выше 4 при медиане 3 — прежняя пятёрка была снята со старого, сквозного определения. Остальные пороги сдвинулись на пределах округления вместе с корпусом.
...
`content-humanizer` получил два новых скрипта. `scripts/scan_rhythm.py` считает то, чего регекс не видит: разброс длины предложений и абзацев, доли коротких и длинных фраз, самый длинный ровный участок, отношение существительных к глаголам (с `pymorphy3`, если установлен). `scripts/calibrate.py` собирает пороги по архиву автора и пишет `scripts/baseline.json` — прежние значения были подобраны на глаз. Общий разбор markdown вынесен в `scripts/rutext.py`.

### Удалено

Скилл `content-zavod` убран из домена `content` вместе с упоминаниями в документации.

### Изменено

`scan_tells.py` переписан после проверки на живом корпусе. Frontmatter, code fences, URL и сноски больше не считаются текстом и не разбавляют плотность. Тире считаются только в прозе: в буллете это разделитель списка. Тэлл «данный» сужен до однозначных форм — множественное число омонимично существительному «данные» и давало ложные срабатывания в технических текстах. Слова, нормальные поодиночке («ключевой», «эффективный», «в качестве», «от X до Y»), переехали из точечных тэллов в агрегатные и срабатывают от трёх вхождений. Добавлен режим `--sections`: общая плотность разбавляет один плохой раздел до «чисто».

Шаги механического скана (0 и 4.5) вернулись в процесс `content-humanizer` — теперь с обоими сканерами и калиброванными порогами.

...
В `~/.agents/skills/` живут не только наши скиллы, но и чужие — установленные из других репозиториев. Они **не вендорятся** в bear-skills. `skills-lock.json` в корне фиксирует, что именно стоит рядом: имя, источник, лицензию и sha256 файла `SKILL.md` на момент фиксации.

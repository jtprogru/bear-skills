...
| `git-release-tag` | Определяет semver-bump из коммитов, генерирует changelog, ставит один тег | «выпустить релиз», «бампнуть версию», «затегать» |
| `git-monorepo-release` | В монорепо решает, какие пакеты бампить и какие теги ставить одновременно; умеет backport | «релиз монорепо», «затегать несколько SDK», «бэкпортнуть фикс в v0.1» |

Два релизных скилла делят работу так: `git-monorepo-release` решает **что** релизить, `git-release-tag` ставит **каждый** тег с changelog. В репозитории с одним пакетом первый не нужен.

### Агенты

- **`git-flow`** — оркестратор полного цикла: ветка → коммиты → PR → тег. Вызывает скиллы домена в нужные моменты.

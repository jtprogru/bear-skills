...
**Шаг 0. Механический скан.** Если текст лежит в файле — прогони оба сканера до чтения:

```bash
python3 scripts/scan_tells.py <файл> --sections
python3 scripts/scan_rhythm.py <файл>
```

`scan_tells` находит вхождения регексного подмножества каталога и считает плотность на 100 слов, в разрезе по секциям. Разбивка важнее общей цифры: длинная статья разбавляет один плохой раздел до «чисто». `scan_rhythm` считает то, чего регекс не видит вообще, — разброс длины предложений и абзацев, долю коротких и длинных фраз, самый длинный ровный участок, отношение существительных к глаголам.

Ни один из них не заменяет чтение: сканеры не видят отсутствия конкретики и эмоциональной плоскости. Но вхождения и дисперсию они не пропускают и токенов не стоят, поэтому глазами ищи то, чего они не умеют, а не то, что уже нашли.

Пороги берутся из `scripts/baseline.json` — он собран по корпусу автора, а не выдуман. Если работаешь с чужим текстом и архив автора доступен, пересобери норму под него:

```bash
python3 scripts/calibrate.py <каталог-с-текстами> --write
```

Подробности, границы применимости и что сканеры сознательно не ловят — в [`references/tells-ru.md`](references/tells-ru.md). Отчёт — ориентир, а не измерение: он показывает, куда смотреть, и не доказывает, что текст машинный.
...
Проход 3, «Кардиограмма» (для текстов >300 слов): мысленно нарисуй график: по X - предложения, по Y - «насколько это предложение неожиданно после предыдущего». У человека график дёргается. У AI - почти прямая. Если график гладкий - вставь 2-3 всплеска: неожиданное сравнение, резкий вопрос, числовой факт среди рассуждений, личную ремарку в скобках. Числовая опора для этого прохода — `sent_cv`, `flat_run` и доли коротких/длинных фраз из `scan_rhythm.py`: `flat_run` прямо показывает, сколько предложений подряд идут одной длиной, и куда именно вставлять всплеск. Считается он внутри куска прозы — через заголовок, список или таблицу цепочка не тянется, поэтому найденный участок лежит в одном месте текста и находится глазами.

Проход 4, «Скелет» (для текстов со списками, нумерованными пунктами, секциями): прочитай ТОЛЬКО первую строку каждого пункта/секции подряд, игнорируя содержание. Это скелет текста. Вопрос: «Скелет звучит как шаблон?» Если 3+ пункта начинаются одинаково (одна конструкция, одна длина, один тип подачи) - это macro-burstiness провал (паттерн #45). Исправь: разные зачины, разная длина блоков, разный тип объяснения. Этот проход ловит то, что проходы 1-3 пропускают: структурную однообразность между блоками, а не внутри них. Косвенные подсказки — `para_cv` и `listicle_share` из `scan_rhythm.py`: ровные по длине абзацы и высокая доля строк-пунктов означают, что текст собран блоками одного размера. Сами зачины пунктов регекс не оценивает, читай их глазами.

**Шаг 4.5. Повторный скан.** Если работал с файлами — прогони сравнение и перемерь ритм:

```bash
python3 scripts/scan_tells.py <до> <после>
python3 scripts/scan_rhythm.py <после>
```
...
Ритм проверяй отдельно: вычистка штампов сама по себе выравнивает текст. Модель правит предложения по одному, приводя их к средней длине, и на выходе разброс падает. Если `sent_cv` или `para_cv` после правки стали ниже, чем были, ты убрал маркеры и добавил гладкости — то есть поменял один признак машины на другой.
...
> - **Считается только парное тире.** `scan_tells.py` меряет вставные конструкции (два тире внутри предложения), а общую плотность печатает справочно, без порога. Замер по архиву блога: у одного и того же автора в технических how-to 0.3 тире на 100 слов, в лонгридах 2.9 — общий счётчик отделяет жанр от жанра, а не человека от модели. Цифра вида «3.3 на 100 слов» сама по себе не находка, и выносить её в отчёт как претензию не надо.

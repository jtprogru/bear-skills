...
description: >
  Создаёт semver-тег для релиза с генерацией changelog из коммитов с прошлого тега.
  Определяет нужный bump (patch/minor/major) по содержимому коммитов, формирует
  release notes. Используй, когда пользователь хочет «выпустить релиз», «бампнуть
  версию», «поставить тег», «сделать релиз», «затегать».
---
...
- монорепо с несколькими SDK или одиночный пакет?
  ...
  - одиночный → `v<X>.<Y>.<Z>`
- если монорепо — спроси пользователя, какой SDK тегаем
